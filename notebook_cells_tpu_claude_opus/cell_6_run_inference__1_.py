...
=============================================================================
  ...
  - Handles both FSDP-sharded and single-core models
...
# ---------------------------------------------------------------------------
_missing = []
for _var in ["tpu_model", "model_tokenizer", "counting_tokenizer",
...
# Check if FSDP is being used (set by Cell 5)
try:
    _use_fsdp = use_fsdp
except NameError:
    _use_fsdp = False
...
def generate_response(html: str) -> Tuple[Optional[str], int]:
    ...
    Handles both FSDP-sharded and single-device models.
    Includes xm.mark_step() to prevent XLA graph explosion.
    """
    ...
    try:
        ...
        # causing HBM to grow linearly until RESOURCE_EXHAUSTED.
        if device_type == "tpu":
        ...
        # Move output to CPU for decoding (avoids keeping large tensors on TPU)
        output_cpu = output.cpu()
        new_ids = output_cpu[0][input_ids.shape[1]:]
        ...
        # Explicit cleanup to free TPU HBM
        del output, output_cpu, new_ids, input_ids
        gc.collect()
        if device_type == "tpu":
            xm.mark_step()  # Flush any pending deallocations
        ...
        if "RESOURCE_EXHAUSTED" in err_str:
            print(f"    OOM ERROR: Input too large for TPU HBM.")
            print(f"    Input tokens: ~{len(input_ids[0])}")
            print(f"    Try reducing MAX_NEW_TOKENS or HTML_BUDGET_RATIO in tpu_config.")
        else:
            print(f"    ERROR: {e}")
        del input_ids
        gc.collect()
